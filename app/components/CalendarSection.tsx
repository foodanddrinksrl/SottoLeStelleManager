      ...
      <div className="card no-print">
        <div className="weekbar">
          <label className="field">
            <span>Dal</span>
            <input type="date" value={weekInfo.start} onChange={(e) => setWeekInfo(makeWeekInfo(new Date(e.target.value + 'T00:00:00')))} />
          </label>
        ...
        {giorni.map((g) => (
          <div className="day" key={g}>
            ...
            <div className="rest no-print">
              <input value={dayRests[g] || ''} onChange={(e) => setDayRests({ ...dayRests, [g]: e.target.value })} placeholder="Riposo" />
            </div>
            {['Pranzo', 'Cena'].map((t) => (
              <div className="shift" key={t}>
                <div className="shift-title">
                  ...
                  <label className="no-print">
                    <input type="checkbox" checked={!!closed[ck(g, t)]} onChange={(e) => setClosed({ ...closed, [ck(g, t)]: e.target.checked })} /> Chiuso
                  </label>
                </div>
                {closed[ck(g, t)] ? (
                  ...
                  reparti.map((r) => {
                    ...
                    return (
                      <div className="reparto" key={r}>
                        <strong>{emoji(r)} {r}</strong>
                        {selected.map((name, i) => (
